...
@section('content')
    <section class="mb-5">
        <div class="mb-4 text-xl font-semibold text-gray-700">
            ...
            <a href="#" class="text-blue-600 hover:underline">Roles</a>
        </div>
        ...
        <form method="GET" action="{{ route('roles') }}">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 items-end">
                ...
                <div class="flex flex-col w-full">
                    <label class="text-xs font-semibold text-gray-700 mb-1">Nama Role</label>
                    <input type="text" name="name" value="{{ request('name') }}" placeholder="Nama Role"
                        class="rounded-md border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-[#5aba6f] focus:outline-none" />
                </div>

                <div class="flex flex-col w-full">
                    <label class="text-xs font-semibold text-gray-700 mb-1">Guard</label>
        ...
        <div class="overflow-hidden rounded-lg border border-gray-400 shadow-sm">
                ...
                <table class="w-full text-sm text-left text-gray-900">
                    <thead class="bg-[#5aba6f]/70 text-gray-900">
                        <tr class="[&>th]:border-b [&>th]:border-gray-500">
                            <th class="px-6 py-4 font-extrabold text-left">Nama Role</th>
                    ...
                    <tbody class="bg-gray-200 divide-y divide-gray-500">
                        @forelse ($roles as $role)
                            <tr class="[&>td]:border-b [&>td]:border-gray-400 hover:bg-gray-100">
                                ...
                                <td class="px-6 py-4 font-semibold">
                                    {{ optional($role->created_at)->timezone(config('app.timezone'))->format('d/m/Y H:i') }}
                                </td>
            ...
            <div
                ...
                <div class="text-xs sm:text-sm font-semibold text-gray-800">
                    Showing {{ $roles->firstItem() ?? 0 }}–{{ $roles->lastItem() ?? 0 }} of {{ $roles->total() }}
                </div>
