    ...
    <div class="p-4 sm:p-8 bg-white min-h-screen" id="printableArea">
        ...
        <div class="border-b-4 border-gray-800 pb-4 mb-6 flex justify-between items-end">
            <div>
                ...
                <p class="text-sm text-gray-600 font-medium">No. Laporan: <span
                        class="font-mono font-bold text-black">PB-{{ str_pad($productionBatch->id, 5, '0', STR_PAD_LEFT) }}</span>
                </p>
            </div>
        ...
        <div class="grid grid-cols-2 gap-8 mb-8 border-b border-gray-200 pb-6">
            ...
            <div class="bg-gray-50 p-4 rounded-xl border border-gray-200">
                <label class="text-[10px] font-black text-[#2E7E3F] uppercase tracking-widest block mb-2">Detail Produk
                    Jadi</label>
                <div class="grid grid-cols-2 gap-2">
                    <span class="text-xs text-gray-500">ID Barang</span>
                    <span class="text-xs font-bold">:
                        {{ $productionBatch->productStock->productVariant->product->code ?? '-' }}</span>

                    <span class="text-xs text-gray-500">SKU</span>
                    <span class="text-xs font-bold text-blue-700">:
                        {{ $productionBatch->productStock->productVariant->sku }}</span>
                    ...
                    <span class="text-lg font-black text-[#2E7E3F]">: {{ $productionBatch->quantity }}
                        {{ $productionBatch->productStock->productVariant->unit }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="mb-8">
            <h3 class="text-xs font-black mb-3 uppercase tracking-[0.2em] text-gray-600">Konsumsi Bahan Baku (Material
                Usage)</h3>
                    ...
                    @foreach ($productionBatch->materials as $index => $material)
                        <tr class="border-b border-gray-300 hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <div class="text-[10px] text-gray-500 font-mono">{{ $material->rawMaterial->code ?? '-' }}
                                </div>
                            </td>
        ...
        <div class="grid grid-cols-3 gap-12 text-center">
            ...
            <div>
                {{-- Logo atau Cap Perusahaan --}}
                <div class="flex items-center justify-center h-full">
                    <div
                        class="w-20 h-20 border-4 border-gray-100 rounded-full flex items-center justify-center opacity-20 rotate-12">
        ...
        <div class="mt-12 pt-4 border-t border-gray-100 hidden print:block">
            <p class="text-[8px] text-gray-400 text-center italic">
                Dokumen ini merupakan laporan resmi sistem pergudangan yang dihasilkan secara otomatis. Segala manipulasi
                data tanpa persetujuan akan diproses sesuai ketentuan perusahaan.
            </p>
        </div>
    </div>
@endsection
